    ...
    public <T> @NotNull LazyOptional<T> getCapability(IToolStackView iToolStackView, Capability<T> capability) {
        if(iToolStackView.getModifierLevel(TConstructMTKModifiers.MTK_MODIFIER.get()) > 0){
            if(iToolStackView.hasTag(TinkerTags.Items.ARMOR)){
                ...
                if(capability == MTKCapabilities.KILL_SWORD || capability == MTKCapabilities.RANGE_BREAK || capability == MTKCapabilities.MULTIPLE || capability == MTKCapabilities.SPREAD_GROW || capability == MTKCapabilities.WOOD_REVERSE){
